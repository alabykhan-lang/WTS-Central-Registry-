...
const RECOVERY_PUBLIC_URL = (process.env.WTS_PORTAL_RECOVERY_URL || process.env.WTS_PORTAL_ORIGIN || 'https://wts-school-platform.vercel.app').replace(/\/$/, '');
const RECOVERY_PATH = process.env.WTS_PORTAL_RECOVERY_PATH || '/portal/account-recovery';
...
async function issueAndSendRecoveryEmail({ login, purpose, reason }) {
  ...
  const url = new URL(RECOVERY_PATH, `${RECOVERY_PUBLIC_URL}/`);
  url.searchParams.set('mode', purpose === 'password_reset' ? 'reset' : 'activation');
  url.searchParams.set('token', issued.token);
  const action = purpose === 'password_reset' ? 'reset your School Portal password' : 'activate your School Portal staff account';
  const subject = purpose === 'password_reset' ? 'Way to Success Standard Schools password reset' : 'Activate your School Portal staff account';
  ...
  const html = `<p>Use the secure link below to ${escapeHtml(action)}.</p><p><a href="${escapeHtml(url.toString())}">Continue to School Portal</a></p><p>This link expires in 30 minutes and can be used once. If you did not request this, ignore this email.</p>`;
