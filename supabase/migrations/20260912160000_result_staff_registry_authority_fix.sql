-- Results staff access must be sourced from the authoritative staff registry.
-- school_people is a shared identity table and may also contain students; its
-- institutional_classification is not sufficient to identify staff.
-- This change does not touch SSO, PKCE, session, or identity exchange logic.
...
begin
  ...
  with registry_staff as (
    select
      ...
      s.staff_number,
      coalesce(nullif(trim(s.designation), ''), 'Staff') as designation,
      coalesce(nullif(trim(s.staff_category), ''), 'staff') as staff_category,
    ...
    where s.registration_status = 'active'
      ...
      and exists (
        select 1
        from public.school_person_roles pr
  ...
  from (
    select
      ...
      case
        ...
        else public.school_result_write_enabled(r.person_id)
