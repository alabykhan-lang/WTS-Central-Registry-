...
module.exports = async function staffSession(req, res) {
  ...
  if (input.action !== 'login') return send(res, 400, { ok: false, code: 'STAFF_SESSION_ACTION_RETIRED' }, clearCookie());
  const login = typeof input.login === 'string' ? input.login.trim() : '';
  ...
  if (!authentication.client_code || !authentication.client_secret) {
    return send(res, 503, { ok: false, code: 'STAFF_SESSION_SERVICE_UNAVAILABLE' }, clearCookie());
  }
