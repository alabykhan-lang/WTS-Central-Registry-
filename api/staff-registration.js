...
function statusFor(code) {
  if (['FULL_NAME_REQUIRED', 'VALID_EMAIL_REQUIRED', 'PHONE_REQUIRED', 'WHATSAPP_NUMBER_INVALID', 'PHOTOGRAPH_INVALID', 'PASSWORD_REQUIREMENTS_NOT_MET'].includes(code)) return 400;
  return 400;
}
...
module.exports = async function staffRegistration(req, res) {
  ...
  if (result?.ok && result.code === 'STAFF_REGISTRATION_SUBMITTED') return send(res, 200, result);
