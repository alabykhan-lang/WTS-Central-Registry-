...
const ROUTES = {
  adminRead: 'school_registry_admin_read_session_api',
  studentWrite: 'school_registry_student_write_session_api',
  staffWrite: 'school_registry_staff_write_session_api',
  guardianWrite: 'school_registry_guardian_write_session_api',
};
...
async function rpc(name, payload) {
  ...
  return response.ok ? result : { ok: false, code: result.code || 'REGISTRY_RECORDS_SERVICE_UNAVAILABLE' };
}

function statusFor(code) {
  if (['RESULT_SESSION_REQUIRED', 'RESULT_SESSION_NOT_ACTIVE', 'RESULT_SESSION_AUDIENCE_MISMATCH', 'PORTAL_ACCESS_NOT_GRANTED', 'IDENTITY_CREDENTIAL_NOT_ACTIVE', 'CENTRAL_IDENTITY_NOT_ACTIVE'].includes(code)) return 401;
  if (['MANAGEMENT_ACCESS_DENIED', 'STAFF_NOT_ACTIVE'].includes(code)) return 403;
  return 400;
}

module.exports = async function registryRecords(req, res) {
  ...
  if (!rpcName || typeof input.action !== 'string') return send(res, 400, { ok: false, code: 'REGISTRY_ACTION_REQUIRED' });
