...
function endAllocation(item,type){
  ...
  form.onsubmit=async(event)=>{event.preventDefault();const reason=$('#allocationEndReason').value.trim();if(reason.length<8){error.textContent='ALLOCATION_REASON_REQUIRED';return;}try{await write(type,{allocationId:pendingAllocationEnd.item.id,reason});pendingAllocationEnd=null;dialog.close();}catch(result){error.textContent=result.code || result.message;}};dialog.showModal();
}
